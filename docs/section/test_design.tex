...
\item Verificar que está dentro del rango de usuario final para esa LAN:
  \begin{itemize}
  \item LAN 1: 172.16.192.1 -- 172.16.195.254
  \item LAN 2: 172.16.196.1 -- 172.16.199.254
  \item LAN 3: 172.16.200.1 -- 172.16.200.254
  \end{itemize}
...
\textbf{Procedimiento:}
\begin{enumerate}
\item Desde Host en LAN 1 (ej: 172.16.192.10), ejecutar \texttt{ping 172.16.196.10}
...
\item Ej: \texttt{ping 2001:dbe:bef9:2::100}
...
\textbf{Procedimiento:}
\begin{enumerate}
\item Desde cliente en LAN 1, abrir navegador y acceder a \texttt{http://172.16.202.1}
\item Capturar página de bienvenida
\item Repetir desde clientes en LAN 2 y LAN 3
\item Abrir navegador y acceder a \texttt{http://[2001:dbe:bef9:10::1]}
\item Ejecutar \texttt{curl} desde Linux: \texttt{curl -v http://172.16.202.1}
\end{enumerate}
...
\textbf{Procedimiento:}
\begin{enumerate}
\item Desde un cliente, ejecutar \texttt{telnet 172.16.202.2 25}
\item Verificar que responde con \texttt{220 SMTP Ready}
\item Ejecutar \texttt{quit}
\item Ejecutar \texttt{telnet 172.16.202.2 143}
\item Verificar que responde con \texttt{* OK Dovecot}
\item Configurar cliente de correo (Thunderbird) apuntando a 172.16.202.2
\item Intenta enviar y recibir correo de prueba
\end{enumerate}

\textbf{Criterio de Éxito:}
\begin{itemize}
\item Puerto 25 responde con mensaje SMTP
\item Puerto 143 responde con mensaje IMAP
\item Cliente de correo puede conectarse y listar buzones
\item Correo de prueba se entrega correctamente
\end{itemize}

\textbf{Herramientas:} \texttt{telnet}, \texttt{nc} (netcat), cliente Thunderbird
...
\item En cliente Windows 1, conectar al servidor: ejecutar cliente con
  \texttt{./chat-client 172.16.202.3 8080}
...
\textbf{Procedimiento:}
\begin{enumerate}
\item Desde cliente, intentar \texttt{telnet 172.16.192.1 23}
\item Verificar que \textbf{no} responde (conexión rechazada o timeout)
\item Ejecutar \texttt{ssh admin@172.16.192.1}
...
\item Intentar conectar a puerto cerrado (ej: 3389 RDP):
  \texttt{telnet 172.16.202.1 3389}
...
\subsection{Resumen de Criterios de Aceptación}

La validación del proyecto se considera exitosa cuando se cumplen \textbf{todos}
los siguientes criterios:

\begin{table}[H]
\centering
\caption{Matriz de aceptación del proyecto}
\label{tab:criterios_aceptacion}
\begin{tabularx}{\textwidth}{l l l}
\toprule
\textbf{Aspecto} & \textbf{Criterio} & \textbf{Estado} \\
\midrule
Direccionamiento IPv4 & Todas las interfaces tienen IPs del plan & --- \\
\midrule
Direccionamiento IPv6 & SLAAC funciona en clientes & --- \\
\midrule
DHCP & Clientes adquieren IPs dinámicas en rango correcto & --- \\
\midrule
Conectividad L3 IPv4 & Ping entre todas las LANs exitoso & --- \\
\midrule
Conectividad L3 IPv6 & Ping IPv6 entre LANs exitoso & --- \\
\midrule
OSPF/OSPFv3 & Routers tienen adyacencia FULL y conocen todas las rutas & --- \\
\midrule
Servidor Web & Accesible desde todas las LANs en IPv4 e IPv6 & --- \\
\midrule
Servidor Correo & Puertos 25 y 143 responden correctamente & --- \\
\midrule
Servidor Chat & Múltiples clientes conectan y se comunican & --- \\
\midrule
Seguridad SSH & SSH habilitado, Telnet deshabilitado & --- \\
\midrule
