...
Dividir los servicios en dos máquinas virtuales responde a una mejora en el aislamiento de fallos, escalabilidad a futuro y prevención de conflictos de puertos o recursos. La asignación fija es necesaria para asegurar que los clientes de todas las LAN puedan localizar los servicios de forma consistente en los registros DNS sin depender de mecanismos dinámicos.

El acceso administrativo a estos servidores se realiza exclusivamente mediante SSHv2. A nivel de SO, se despliega una política de \textit{firewall} restrictiva perimetral usando UFW, que expone únicamente los puertos correspondientes a los servicios públicos. Esta configuración reduce la superficie de ataque y aísla los procesos internos que comunican ambas VM en el \textit{backend}.
